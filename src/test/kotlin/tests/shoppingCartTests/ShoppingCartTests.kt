...
class ShoppingCartTests : BaseTest() {
    ...
    fun `Verify user can add a product in the shopping cart`() {
        ...
        assertTrue(
            shoppingCartPage.isCorrectAddedSizeDisplayedOnCart(productPage.getProductSize()),
            "Correct size added to the shopping cart correctly"
        )
    }
    ...
    fun `Verify user can update the shopping cart`() {
        ...
        assertTrue(
            shoppingCartPage.verifyCartUpdate(productPrice),
            "Shopping cart is updated correctly"
        )
    }
